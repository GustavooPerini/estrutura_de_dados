...
void busca_UCS(Vector *cidades, int cidadeInicio, int cidadeAlvo){
    ...
    heap_push(fronteira, vector_get(cidades, cidadeInicio), 0);
    int achouCidade = 0;
        ...
        if(result){
            ...
            for(int i = 0; i < get_num_vizinhos(cidadeVisitada); i++){
                ...
                if(jaVisitou == 0){ //se ele não visitou a cidade vizinha, pode por na fronteira
                    ...
                    set_custo_origem(cidadeVizinha, get_custo_origem(cidadeVisitada) + distVizinho);

                    heap_push(fronteira, cidadeVizinha, get_custo_origem(cidadeVizinha));//boto a cidade vizinha na fronteira
                }
            }
        }
    }
